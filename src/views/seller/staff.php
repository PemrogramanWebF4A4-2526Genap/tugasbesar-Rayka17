...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $mitra) {
    ...
    if ($action === 'create') {
        ...
        if ($check) {
            header("Location: staff.php?email_exists=1");
            exit;
        }
        ...
        if (mysqli_error($conn)) {
            die('SQL Create User Petugas Error: ' . mysqli_error($conn));
        }
        ...
        if (mysqli_error($conn)) {
            die('SQL Create Staff Error: ' . mysqli_error($conn));
        }
    ...
    if ($action === 'update') {
        ...
        $staff_user_id = (int) ($_POST['user_id'] ?? 0);

        $fullname = mysqli_real_escape_string($conn, $_POST['fullname'] ?? '');
        $email = mysqli_real_escape_string($conn, $_POST['email'] ?? '');
        $phone = mysqli_real_escape_string($conn, $_POST['phone'] ?? '');
        $address = mysqli_real_escape_string($conn, $_POST['address'] ?? '');
        $staff_status = mysqli_real_escape_string($conn, $_POST['status'] ?? 'active');

        $allowedStatus = ['active', 'inactive'];

        if ($staff_id <= 0 || $staff_user_id <= 0 || $fullname === '' || $email === '' || !in_array($staff_status, $allowedStatus)) {
            header("Location: staff.php?error=1");
            exit;
        }
    ...
    if ($action === 'reset_password') {
        ...
        header("Location: staff.php?reset=1");
        exit;
    }
...
$staffs = mysqli_query($conn, "
    ...
    WHERE staff.mitra_id='$mitra_id'
    ORDER BY staff.id DESC
");

function staffBadgeSeller($status)
{
    if ($status === 'active') {
...
<main class="dashboard-main">
    ...
    <section style="padding:26px;">
        ...
        <div style="margin-bottom:22px;">
            ...
            <p class="page-subtitle">Tambah, edit, nonaktifkan, dan reset password petugas milik seller.</p>
        </div>
        ...
        <?php if (isset($_GET['reset'])) : ?>
            <div style="background:#dcfce7;color:#166534;border-radius:18px;padding:14px 18px;font-weight:800;margin-bottom:20px;">
                Password petugas berhasil direset menjadi 123456.
            </div>
        <?php endif; ?>
        ...
        <?php if (isset($_GET['email_exists'])) : ?>
            <div style="background:#fee2e2;color:#b91c1c;border-radius:18px;padding:14px 18px;font-weight:800;margin-bottom:20px;">
                Email petugas sudah digunakan.
            </div>
        <?php endif; ?>

        <?php if (isset($_GET['error']) || $error !== "") : ?>
            <div style="background:#fee2e2;color:#b91c1c;border-radius:18px;padding:14px 18px;font-weight:800;margin-bottom:20px;">
                <?= $error !== "" ? htmlspecialchars($error) : "Gagal memproses data petugas."; ?>
            </div>
        <?php endif; ?>
                ...
                <?php else : ?>
                    ...
                    <div class="modern-card" style="padding:36px;text-align:center;">
                        ...
                        <p style="color:#64748b;">
                            Tambahkan petugas untuk membantu operasional laundry.
                        </p>
                    </div>
